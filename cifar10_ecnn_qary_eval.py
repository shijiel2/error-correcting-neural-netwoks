...
depth_val = args.resnet_depth
stack_val, res_block_val = 2, 1 # Split point for shared backbone in SubNetResNet
nbits_per_subnet_val = FLAGS.num_models # This was 'nbits_per_subnet' in Keras.
...
if __name__ == "__main__":
    ...
    q_val = 2 # Number of classes (q), e.g., 2 for binary, 10 for CIFAR-10
    ...
    model_filepath_to_load = args.saved_model_path
    ...
    try:
        # The saved state_dict is for the backbone model (SubNetResNet), not the
        # full EnsembleModelWithFinalDecoder. We need to load it into the `backbone` attribute.
        state_dict = torch.load(model_filepath_to_load, map_location=device)

        # Get the underlying model, unwrapping from DataParallel if needed.
        model_to_load = eval_model.module if isinstance(eval_model, nn.DataParallel) else eval_model
        
        # Load the state dict into the backbone component.
        model_to_load.backbone.load_state_dict(state_dict)
    ...
    gammas = [0.0]  # For now, only clean evaluation; can add adversarial attacks later
    for gamma_val in gammas: # Loop over attack strengths
        ...
        print(f"    Accuracy on (gamma={gamma_val}): {accuracy:.2f}% ({correct_predictions}/{total_samples})")
